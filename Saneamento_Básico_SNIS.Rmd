...
```{r}
san_mt <- read.csv("https://github.com/CamilyNunes/produto_snis_mt/raw/refs/heads/main/Dados/SNIS%20-%20S%C3%A9rie%20Hist%C3%B3rica/Agregado-20240925161723-SNISMT.csv", sep = ";", encoding = "UTF-8")
```
...
## 4.2 Indicadores de Saneamento

Os indicadores de saneamento são fundamentais para avaliar o progresso em direção às metas do ODS 6. Abaixo estão apresentados alguns dos principais indicadores para o município de Rondonópolis:

-   População Atendida com Abastecimento de Água (Pop_atendida_abas_agua)

-    Consumo de Água Per Capita (Consumo_agua_percapita)

-    Índice de Coleta de Esgoto (Índice_coleta_esgoto)

-   Índice de Tratamento de Esgoto (Índice_tratamento_esgoto_percentual)

-   Volume de Esgoto Coletado (Volume_esgoto_coletado_m3)

    ```{r}
# Selecionar e visualizar os principais indicadores
indicadores <- san_roo %>% 
      select(Ano, Pop_atendida_abas_agua, Consumo_agua_percapita, 
             Índice_coleta_esgoto,
             Índice_tratamento_esgoto_percentual, 
             Volume_esgoto_coletado_m3)

# Exibir a tabela dos indicadores
indicadores
    ```
...
Rondonópolis está proximo de bater a meta?
