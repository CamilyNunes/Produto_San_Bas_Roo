...
```{r}
url <- "https://github.com/CamilyNunes/produto_snis_mt/raw/refs/heads/main/Dados/SNIS%20-%20S%C3%A9rie%20Hist%C3%B3rica/agregado_snis_brasil.csv"  

san_br <- read_delim(url, delim = ";", locale = locale(encoding = "ISO-8859-1"))
```
  
```{r}
san_br <- read_csv(url, locale = locale(encoding = "ISO-8859-1"))
```
...
```{r}
san$pop_atendida_abas_agua <- san_br$`AG001 - População total atendida com abastecimento de água`

san$consumo_agua_percapita <- san_br$`IN022 - Consumo médio percapita de água`

san$índice_coleta_esgoto <- san_br$`IN015 - Índice de coleta de esgoto`

san$índice_tratamento_esgoto_percentual <- san_br$`IN016 - Índice de tratamento de esgoto`

san$volume_esgoto_coletado_m3 <- san_br$`ES005 - Volume de esgotos coletado`
```
