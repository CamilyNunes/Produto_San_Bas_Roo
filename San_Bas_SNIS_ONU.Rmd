...
```{r}
#teste1
calc_missing_proportion_by_year <- function(df, year_col) {
  # Verificar se a coluna do ano existe
  if (!year_col %in% names(df)) {
    stop("Coluna do ano não encontrada no conjunto de dados.")
  }
  
  df %>%
    group_by(!!sym(year_col)) %>% # Agrupa por ano
    summarise(across(everything(), ~ mean(is.na(.)), .names = "Omissos_{.col}")) %>%
    pivot_longer(cols = -!!sym(year_col), 
                 names_to = "Atributo", 
                 values_to = "Omissos") %>%
    filter(Omissos > 0) %>%
    mutate(Atributo = sub("Omissos_", "", Atributo)) # Limpa o nome das colunas
}
  
missing_proportions_by_year <- calc_missing_proportion_by_year(san_roo, "Ano")
  
plot_data_by_year <- missing_proportions_by_year %>%
  mutate(Presentes = 1 - Omissos) %>%
  pivot_longer(cols = c(Omissos, Presentes), 
               names_to = "Valores", 
               values_to = "Proporcao")
  
gg_by_year <- ggplot(plot_data_by_year, aes(x = Ano, 
                                            y = Proporcao, 
                                            fill = Valores)) +
  geom_bar(stat = "identity", position = "fill") +
  scale_y_continuous(labels = scales::percent) +
  labs(
    title = "Proporção de Valores Omissos por Ano",
    x = "Ano",
    y = "Proporção (%)") +
  scale_fill_manual(values = c("Omissos" = cor_omisso, "Presentes" = cor_presente), 
                    labels = c("Valores Presentes", "Valores Omissos")) + 
...
ggsave(
  filename = 'grafico_proporcao_omissos_por_ano.png', 
  plot = gg_by_year, 
  path = 'resultados',
  width = 10,
  height = 6,
...
interactive_plot_by_year <- ggplotly(gg_by_year)   
  
interactive_plot_by_year
``` 
...
calc_missing_by_year <- function(df) {
  ...
  df_long <- numeric_cols %>%
    ...
    summarise(
      ProporcaoOmissos = mean(is.na(Valor)),
      ProporcaoPresentes = 1 - mean(is.na(Valor)),
      .groups = "drop"
    )
  return(df_long)
}
...
plot_data <- missing_by_year %>%
  pivot_longer(cols = c(ProporcaoOmissos, ProporcaoPresentes),
               names_to = "Tipo",
               values_to = "Proporcao")
...
gg <- ggplot(plot_data, aes(x = Ano, 
  ...
  scale_y_continuous(labels = scales::percent) +
  scale_fill_manual(
    values = c("ProporcaoOmissos" = cor_omisso, "ProporcaoPresentes" = cor_presente),
...
calc_missing_by_year <- function(df) {
  ...
  # Adicionar uma coluna para agrupar as variáveis em pares
  df_long <- df_long %>%
    mutate(Grupo = rep(1:ceiling(n_distinct(df_long$Variavel)/2), each = 2)[1:n()])
...
gg <- ggplot(plot_data, aes(x = Ano, 
                            ...
                            fill = Tipo)) +
  geom_bar(stat = "identity", position = "fill") +
  facet_wrap(~Grupo, scales = "free_y", ncol = 2) +  # Alterado para mostrar em pares (duas colunas)
  scale_y_continuous(labels = scales::percent) +
  scale_fill_manual(
...
ggsave(
  ...
  plot = gg,
  path = "resultados",
  width = 18,  # Aumentando a largura
  height = 10, # Aumentando a altura
  units = "in")
