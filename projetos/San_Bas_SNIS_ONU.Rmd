---
title: Avaliação dos Avanços de Rondonópolis (MT) em Relação ao Objetivo de Desenvolvimento
  Sustentável 6 (ODS 6) – Água Potável e Saneamento
...
```{r,fig.align='center', fig.width=9, fig.height=14, warning=FALSE,echo=FALSE,message=FALSE}
...
tema_plot <- function(dataTime, legendavar, chave) {
  ...
  plot <- ggplot(data = dataTime, aes(x = tempo, y = x, group = 1)) +
            ...
            geom_label(aes(label = label), size = 3, color = "#3586ff", nudge_y = 0.5) +  # Cor do Rótulos
            labs(
...
Inicialemnte, é possivel verificar que em 2021 de 100% da população total 96,19% da população urbana foi atendida com abastecimento de água.
